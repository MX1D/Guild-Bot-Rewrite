...
export default {
    excute: async function (bot, text, client) {
        ...
        const purse = skycrypt?.data?.purse?.toLocaleString("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }) || 0;

        const info = [{ title: "Skill average:", description: skillAvg }, { title: "Catacombs level:", description: cataLvl }, { title: "Senither/Lily Weight:", description: `${ Math.round(senitherWeight) }/${ Math.round(lilyWeight) }` }, { title: "Slayers:", description: `${ zombie }/${ spider }/${ wolf }/${ enderman }` }, { title: "Slayer exp:", description: totalSlayer }, { title: "Skills exp:", description: totalSkill }, { title: "HOTM:", description: hotmLvl }, { title: "Purse:", description: purse }];
